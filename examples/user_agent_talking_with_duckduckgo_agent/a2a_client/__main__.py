        ...
        try:
            ...
            if response.error:
                print(
                    colorize(
                        C_RED,
                        f"Error from agent (Code: {response.error.code}): {response.error.message}",  # noqa: E501
                    )
                )
                ...
                print(
                    colorize(
                        C_GREEN,
                        f"Task {task_result.id} finished with state: {task_result.status.state}",  # noqa: E501
                    )
                )

                final_answer = "Agent did not provide a final artifact."
                # Extract answer from artifacts (as implemented in PocketFlowTaskManager)  # noqa: E501
                if task_result.artifacts:
                    try:
                        ...
                        if first_text_part:
                            ...
                            final_answer = (
                                f"(Non-text artifact received: {first_artifact.parts})"
                            )
                    ...
                    try:
                        first_text_part = next(
                            (
                                p
                                for p in task_result.status.message.parts
                                if isinstance(p, TextPart)
                            ),
                            None,
                        )
                        if first_text_part:
                            final_answer = (
                                f"(Final status message: {first_text_part.text})"
                            )
                ...
                print(
                    colorize(C_YELLOW, "Received response with no result and no error.")
                )
